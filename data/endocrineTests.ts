...
export const endocrineTests: EndocrineTest[] = [
  ...
  {
    id: "cortisol-diurnal",
    name: "コルチゾール日内変動・深夜唾液コルチゾール",
    tagline: "採血時刻で値が大きく変わる — 正しいタイミングで評価する",
    essence: "HPA軸の概日リズムと採血タイミング",
    color: "blue",
    category: "副腎・HPA軸",
    status: "available",
    what: "コルチゾールは視床下部CRHパルスジェネレーターの概日リズムに従い、早朝に最高値・深夜に最低値をとる。この日内変動を知らずに採血すると「高値」「低値」を誤解する。クッシング症候群では深夜の抑制が消失する（深夜コルチゾール高値）のが最大の特徴。また深夜唾液コルチゾールは外来で簡便に施行できるクッシングスクリーニングとして有用。",
    indications: [
      "コルチゾール値の解釈（採血時刻の確認）",
      "クッシング症候群のスクリーニング（深夜唾液コルチゾール）",
      "副腎不全の評価（早朝8時採血が標準）",
    ],
    preparations: [
      "通常の血中コルチゾール測定は早朝6〜8時が基準（最高値帯）",
      "深夜唾液コルチゾール: 23時〜深夜0時に専用チューブに唾液を採取",
      "唾液採取の30分前は飲食・歯磨き・喫煙を避ける",
      "2日間連続で採取すると精度が上がる（変動があるため）",
    ],
    timeline: [
      ...
      { time: "23〜24時（深夜）", action: "コルチゾール 最低値帯: <1.8 μg/dL が正常", isKey: true, note: "クッシング症候群では抑制されず高値が持続する" },
    ],
    judgments: [
      {
      ...
      {
        parameter: "深夜（23〜24時）唾液コルチゾール（クッシング評価）",
        unit: "μg/dL", threshold: "0.13", isNormalAbove: false,
        normalLabel: "正常（深夜抑制あり）",
        abnormalLabel: "≥0.13 → クッシング症候群を疑う",
        note: "2回測定を推奨。感度92〜100%・特異度93〜100%（最も感度の高いスクリーニング）",
      },
    ],
    normalInterpretation: "早朝高値・深夜低値のリズムが正常に保たれている → HPA軸の概日リズム正常。",
    abnormalInterpretation: "深夜コルチゾールが抑制されない（日内変動の平坦化） → クッシング症候群の特徴。1mg DSTやUFCと組み合わせて診断を進める。",
    cautions: [
      "午後・夕方の採血でコルチゾールが低値でも正常 → 時刻を必ず記録する",
      "唾液採取は歯科処置・口腔内出血があると血液混入で偽高値になる",
    ...
    pitfalls: [
      ...
      "深夜採血は入院患者でないと困難 → 外来では深夜唾液コルチゾールを選択する",
      "唾液採取後は冷凍保存が必要 → 検体の取り扱いを事前に確認する",
    ],
    reportPhrase: "早朝（___時）コルチゾール ___μg/dL。深夜唾液コルチゾール ___μg/dL（【正常範囲／高値】）。",
    hormoneFlow: {
